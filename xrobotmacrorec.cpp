/*****************************************************************************
 *
 * xrobotmacrorec - a fork of xmacro that records the delay between 
 * X mouse and key events.
...
std::chrono::time_point<std::chrono::system_clock> glStopwatchStart; // global stopwatch variable
...
void usage (const int exitCode) {
  ...
  cerr << "  -s  FACTOR  scalefactor for coordinates. Default: 1.0." << endl
	   ...
	   << "              If a keycode is not provided, you will be " << endl
	   << "              asked to select a key." << endl
	   << "  -v          show version. " << endl
...
int findQuitKey (Display * Dpy, int Screen) { 
  ...
  cerr << "The chosen quit-key has the keycode: " << EKey.keycode << endl << endl;
...
void eventCallback(XPointer priv, XRecordInterceptData *d)
{
  ...
  cout << "Delay " << getElapsedTime(glStopwatchStart) << endl;  // print the delay
  glStopwatchStart = std::chrono::high_resolution_clock::now();  // reset the stopwatch
  ...
  switch (type) {
	...
	case KeyPress:
	  ...
	  break;
	default: 
	  cerr<<"Warning: Unknown event type. Type is: "<<type;
	  break;
...
void eventLoop (Display * LocalDpy, int LocalScreen,
  ...
  cerr << "Initialization complete." << endl << endl;
  
  glStopwatchStart = std::chrono::high_resolution_clock::now(); // initialize the stopwatch
  cerr << "Recording has begun..." << endl;
...
//  XCloseDisplay ( RecDpy );
  ...
  cerr << PROG << ": Exiting. Recording has finished." << endl;
