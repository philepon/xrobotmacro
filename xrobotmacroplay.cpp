/*****************************************************************************
 *
 * xrobotmacroplay - a fork of xmacro that plays the recorded delay 
 * between X mouse and key events.
 * Licensed under GPLv2 or any later version.
 *
 ****************************************************************************/

// original xmacro licence:
/***************************************************************************** 
 *
 ...
 * Globals...
 ****************************************************************************/
int   Delay = DefaultDelay; // Note: This is the value of the SIMULATED delay. It is not the delay that gets read from stdin.
...
void usage (const int exitCode) {
  ...
  cerr << "Options: " << endl;
  cerr << "  -d  DELAY   Extra delay in milliseconds for events sent to remote display." << endl
	   << "              Default: 0ms. "
	   << endl
  ...
  cerr << endl;
  cerr << "Example reading from a file:" << endl << endl;
  cerr << "       " << PROG << " :0 < inputFile.txt" << endl << endl;
  cerr << "Example reading from a file w/ scale factor of 0.5:" << endl << endl;
  cerr << "       " << PROG << " -s 0.5 :0 < inputFile.txt" << endl << endl;
  cerr << endl;
...
void parseCommandLine (int argc, char * argv[]) {
  ...
  while ( Index < argc ) {
	...
	// is this '-d'?
	  ...
	  cerr << "Warning: Providing a delay value will cause an extra" << endl
	  	   << "delay to be played before every Key/Mouse event" << endl
	  	   << "in addition to any delay that will be supplied" << endl
	  	   << "via stdin. This may not be the intended behavior." << endl;
	  Index++;
	}
...
float getElapsedTime(const std::chrono::time_point<std::chrono::system_clock> prevTimerPoint)
{
	std::chrono::time_point<std::chrono::system_clock> currentTimerPoint = std::chrono::high_resolution_clock::now();
	return std::abs(static_cast<float>(std::chrono::duration_cast<std::chrono::nanoseconds>(currentTimerPoint-prevTimerPoint).count()/1000000.0));
}

void preciseSleep(float sleepTimeMilliseconds){
	std::chrono::time_point<std::chrono::system_clock> stopWatchStart = std::chrono::high_resolution_clock::now();
	while (getElapsedTime(stopWatchStart) < sleepTimeMilliseconds) {}; // do nothing as we wait
}
